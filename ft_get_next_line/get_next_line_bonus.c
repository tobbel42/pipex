...
/*   Updated: 2021/07/20 12:20:32 by tgrossma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	eof_flag = 1;
	if (temp[fd] == NULL)
		eof_flag = gnl_init_temp(&temp[fd], fd);
	if (eof_flag == -1)
		return (NULL);
	while (eof_flag == 1 && temp[fd][gnl_find_char(temp[fd], '\n')] == '\0')
		eof_flag = gnl_append_read(&temp[fd], fd);
	if (eof_flag == -1)
		return (NULL);
	ret_ptr = gnl_pop_line(&temp[fd]);
	if (temp[fd][0] == 0)
	{
		free(temp[fd]);
		temp[fd] = NULL;
